...
// LORA-09: Send actuator command (Admin only)
router.post('/command', authorize('admin'), async (req, res) => {
  try {
    ...
    const [devices] = await pool.execute('SELECT id, node_id FROM devices WHERE id = ? OR node_id = ?', [device_id, device_id]);
    if (devices.length === 0) {
    ...
    publishMQTT(`tambak/${nodeId}/actuator/command`, { aksi, command_id: result.insertId });
